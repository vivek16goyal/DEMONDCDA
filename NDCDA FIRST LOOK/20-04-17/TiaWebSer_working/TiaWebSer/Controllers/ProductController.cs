...
namespace TiaWebSer.Controllers
{
    public class ProductController : ApiController
    {
        ...
        //CHNAGED ADDED TWO IM COMPANY AND MAKE
        [HttpGet]
        public IEnumerable<ItemMasterWsale> GetItemConsumer(string Iname, string PCODE)
        {
            try
            {
                ...
                foreach (var item in list_itemWsale)
                {
                    ...
                    IM obj_im = gbc_con.Db_itm.SingleOrDefault(p => p.ICODE == item.ICODE);
                    item.company = getcompany(obj_im.CCODE);
                    item.making = getmaking(obj_im.MAKE);
                    item.party = getPARTY(obj_im.CCODE);
                    item.PCODE = obj_im.CCODE;
                }
        ...
        //change for the geting the make
        public string getPARTY(string PCODE)
        {
            ...
            try
            {
                ...
                DbConn GbcCon = new DbConn();
                string name = GbcCon.Database.Connection.Database + ".dbo.";
                DataTable dt = obj.getDataTable(false, "SELECT MMST.CODE1 FROM " + name + "MMST MMST WHERE PCode='" + SelICode + "' ");
                if (dt.Rows.Count != 0)
                {
                    for (int i = 0; i >= dt.Rows.Count;i++)
                    {
                        ...
                        if (dr != null)
                        {
                            GenericName = (dr.IsNull("CODE1")) ? "" : dr["CODE1"].ToString() + "/" + GenericName;
                        }
                        else
                        {
                            GenericName = "";
         ...
         public string getmaking(string MAKE){
             Global obj = new Global();
             string SelICode = "";
             DataRow dr = null;
             string GenericName = "";
             try
             {
                 ...
                 GBCDbConn GbcCon = new GBCDbConn();
                 string name = GbcCon.Database.Connection.Database + ".dbo.";
                 DataTable dt = obj.getDataTable(false, "SELECT CM.CNAME FROM " + name + "CM CM WHERE CCODE='" + SelICode + "' ");
                 if (dt.Rows.Count != 0)
                 {
                     dr = dt.Rows[0];
                 }
                 ...
                 if (dr != null)
                 {
                     GenericName = (dr.IsNull("CNAME")) ? "" : dr["CNAME"].ToString();
                 }
                 else
                 {
                     GenericName = "";
        ...
        public string getcompany(string CCODE) {
